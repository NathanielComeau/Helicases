...
static HashItem *HashItemCreate(HashTable *h) {
    ...
    hi = (HashItem *)(h->options & HASH_POOL_ITEMS
...
HashFile *HashFileFopen(FILE *fp) {
    ...
    if (hf->narchives) {
	hf->archives = (char **)malloc(hf->narchives * sizeof(char *));
	hf->afp = (FILE **)calloc(hf->narchives, sizeof(FILE *));
    } else {
	hf->archives = NULL;
	hf->afp = &hf->hfp;
    }
	...
	for (i = 0; i < hf->narchives; i++) {
	    fnamelen = fgetc(fp);
	    hf->archives[i] = (char *)malloc(fnamelen+1);
	    fread(hf->archives[i], 1, fnamelen, fp);
	    hf->archives[i][fnamelen] = 0;
...
HashFile *HashFileOpen(char *fname) {
	...
	hf->afp = (FILE **)malloc(hf->narchives * sizeof(FILE *));
	if (hf->afp == NULL)
	    return NULL;
